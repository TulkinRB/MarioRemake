...
def create_player(x, y, speed_x, speed_y, max_jump_height, img):
  p = Player()
  p.images = img
  p.img = img[0]
  p.x1 = x
  ...
  p.speed_x = speed_x
  p.speed_y = speed_y
  p.max_jump_height = max_jump_height
  return p
...
def reset():
  ...
  global sec_count
  global view_range
  player = create_player(0, 0, 0.5, 1.5, 70, mario)
  entities = [
...
def new_frame(k_left, k_right, k_up):
  global player
  global entities
  global blocks
  global screen
  ...
  return {'screen':screen, 'finished':(player.x1 >= castle_x)}